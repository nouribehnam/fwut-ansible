#!/bin/bash
config_dir=/opt/router

for i in $(find "${config_dir}/helpers" -type f -iname "*.func"); do
        . "$i"
done
...
function __apply_ipsets {
        rc=0
        for i in $(find ${config_dir} -type f -iname "*.ipset"); do
                echo "Applying $i ..."
...
function __apply_iptables {
        rc=0
        for i in $(find ${config_dir} -type f -iname "*.v4.rules"); do
                echo "Applying $i ..."
        ...
        for i in $(find ${config_dir} -type f -iname "*.v6.rules"); do
                echo "Applying $i ..."
